...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://imagein-4cgn.onrender.com/api/v1/dalle", {
          method: "POST",
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true)
      try {
        const response = await fetch("https://imagein-4cgn.onrender.com/api/v1/post", {
          method: 'POST',
          headers: {
  ...
  return (
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[16px] max-[500px]">
